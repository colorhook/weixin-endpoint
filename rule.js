...
module.exports = function(webot) {
    ...
    webot.set('/^(你个|我)*(sb|二货|擦|你妹|去|傻|傻鸟|煞笔|shabi|傻比|傻逼|神经|(神经|蛇精)病|深井冰|操|草|日|屮|艹)([\.!！。]*)$/i', ["你个傻X", "不许骂人", "你是神经病啊", "深井冰"]);

    webot.set('为人民服务', ["你是活雷锋"]);

    webot.set('毛主席万岁', '万岁万岁万万岁');

    webot.set('/你?好?(nb|nx|牛叉|棒|厉害|赞|great|牛逼)([\.!！。]*)$/', ['谢谢夸奖', '彼此彼此', '还需要努力']);

    webot.set('/^(哈哈|呵呵|haha|hehe|ha|he)([\.!！。]*)$/i', ['笑而不语', '干嘛傻笑','😊', '😂']);
    
    webot.set('/^(yes|Y|非常好|love|like|👍|有趣|神奇|好玩|不错|支持|点赞|有点意思|还行|凑合)$/i', ['嘻嘻', '谢谢鼓励']);
    
    webot.set('/^(no|N|弱|弱爆了?|不行|垃圾|挫|差劲?|太差了?)$/i', ['伤心', '看来还需要很努力才行']);
